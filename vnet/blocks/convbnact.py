...
class BottConvBnAct2d(nn.Module):
    """Bottle neck structure"""

    def __init__(self, channels, ratio, norm_type=nn.BatchNorm2d, act_type=nn.ReLU, final_act=False):
        super(BottConvBnAct2d, self).__init__()
        self.conv1 = ConvBnAct2d(channels, channels // ratio, kernel_size=1, padding=0, norm_type=norm_type,
                                 act_type=act_type)
        self.conv2 = ConvBnAct2d(channels // ratio, channels // ratio, kernel_size=3, padding=1, norm_type=norm_type,
                                 act_type=act_type)
        if not final_act:
            self.conv3 = ConvBnAct2d(channels // ratio, channels, kernel_size=1, padding=0, norm_type=norm_type,
                                     act_type=False)
        else:
            self.conv3 = ConvBnAct2d(channels // ratio, channels, kernel_size=1, padding=0, norm_type=norm_type,
                                     act_type=act_type)
